...
class ReactComp extends Component {
    ...
    updateReservations(props) {
        ...
        if (
            ...
            this.list.scrollToOffset({
                offset: reservations.scrollPosition,
                animated: true,
            });
        }
    ...
    getReservations(props) {
        ...
        const selected = new Date(iterator);
        const dayOfWeek = getDay(selected);
        let scrollPosition = 0;

        if (dayOfWeek < 6) {
            const firstWeekDay = subDays(selected, dayOfWeek);
            iterator = XDate(firstWeekDay);
        }
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const current = new Date(iterator);
            if (current.toDateString() === selected.toDateString()) {
                scrollPosition = reservations.length * 120;
            }
            iterator.addDays(1);
        }
    ...
    render() {
        if (
            ...
            !this.props.reservations[
                this.props.selectedDay.toString('yyyy-MM-dd')
            ]
        ) {
